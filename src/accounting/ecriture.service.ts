...
export class EcritureService {
  ...
  async exportEntries(
    ...
    if (format === 'pdf') {
      ...
      entries.forEach((e) => {
        doc.text(
          `${e.date_ecriture} | ${e.compte_numero} | ${e.compte_intitule} | Débit: ${e.debit} | Crédit: ${e.credit}`,
        );
      });
      doc.end();
  ...
  async getBalance() {
    ...
    const balance: Record<
      string,
      { compte_intitule: string; debit: number; credit: number; solde: number }
    > = {};
    for (const entry of entries) {
