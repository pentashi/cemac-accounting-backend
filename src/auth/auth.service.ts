...
export class AuthService {
  ...
  async envoyerCodeVerification({
    ...
    const now = Date.now();
    if (
      this.derniereDemandeCode[user.id] &&
      now - this.derniereDemandeCode[user.id] < DELAI_RESEND
    ) {
    ...
    return this.envoyerCode(
      canal,
      canal === 'email' ? user.emailProfessionnel : user.telephone,
      code,
    );
  }
  ...
  async demanderResetMdp({
    ...
    const now = Date.now();
    if (
      this.derniereDemandeCode['reset_' + user.id] &&
      now - this.derniereDemandeCode['reset_' + user.id] < DELAI_RESEND
    ) {
      throw new Error('Veuillez patienter avant de redemander un code');
    }
    const code = this.genererCode();
    ...
    const canalEnvoi: 'email' | 'whatsapp' =
      canal === 'sms' ? 'whatsapp' : canal;
    return this.envoyerCode(
      canalEnvoi,
      canalEnvoi === 'email' ? user.emailProfessionnel : user.telephone,
      code,
    );
  }
